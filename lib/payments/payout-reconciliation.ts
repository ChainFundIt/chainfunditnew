...
export async function reconcilePayoutStatus(payoutId: string) {
  try {
    ...
    if (payout.status === 'completed' || payout.status === 'failed') {
      return { success: true, message: 'Payout already finalized', status: payout.status };
    }
    ...
    if (payout.payoutProvider === 'stripe') {
      try {
        ...
        try {
          const transfer = await stripe.transfers.retrieve(payout.transactionId);
          providerStatus = transfer.reversed ? 'failed' : 
                          transfer.status === 'paid' ? 'completed' : 
          ...
          try {
            const stripePayoutObj = await stripe.payouts.retrieve(payout.transactionId);
            providerStatus = stripePayoutObj.status === 'paid' ? 'completed' : 
                           stripePayoutObj.status === 'failed' ? 'failed' : 
                           stripePayoutObj.status === 'canceled' ? 'failed' : 'processing';
            shouldUpdate = providerStatus !== payout.status;
          } catch {
      ...
      try {
        ...
        providerStatus = transfer.data.status === 'success' ? 'completed' : 
                        transfer.data.status === 'failed' ? 'failed' : 'processing';
        shouldUpdate = providerStatus !== payout.status;
      } catch (error) {
    ...
    return { success: true, message: 'Status matches provider', status: payout.status };
  } catch (error) {
