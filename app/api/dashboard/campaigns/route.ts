...
import { db, normalizeEmail, withRetry } from '@/lib/db';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const debugCampaigns = await withRetry(() =>
      db
        .select({
          id: campaigns.id,
    ...
    const userCampaigns = await withRetry(() =>
      db
        .select({
          id: campaigns.id,
          slug: campaigns.slug,
        ...
        .groupBy(
          ...
          campaigns.isChained,
          campaigns.isVerified,
          campaigns.verifiedPendingAt,
          campaigns.verifiedRulesAcceptedAt,
    ...
    const campaignsWithStats = userCampaigns.map((campaign) => {
      ...
      // The database value is authoritative and should match the sum of convertedAmount from donations
      ...
      // Log if there's a significant discrepancy for debugging (but don't auto-fix in GET request)
      const discrepancy = Math.abs(currentAmount - calculatedTotalRaised);
      if (discrepancy > 1000 || (discrepancy > 100 && (discrepancy / Math.max(currentAmount, calculatedTotalRaised, 1)) > 0.05)) {
        console.warn(`Campaign ${campaign.id} amount discrepancy: DB=${currentAmount}, Calculated=${calculatedTotalRaised}, Diff=${discrepancy}`);
      }
