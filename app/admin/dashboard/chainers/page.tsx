...
export default function ChainersPage() {
  ...
  const fetchChainers = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch chainers');
      ...
      toast.error('Failed to fetch chainers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBulkAction = async (action: string) => {
    if (selectedChainers.length === 0) {
      toast.error('Please select chainers to perform bulk action');
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading chainers...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Chainer Management</h1>
          <p className="text-gray-600">Manage chainers, track commissions, and detect fraud</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Chainers</CardTitle>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Chainers</CardTitle>
          <CardDescription>
            Manage chainer accounts and monitor their performance
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-12">
                  <input
                    type="checkbox"
                    checked={selectedChainers.length === chainers.length && chainers.length > 0}
                    onChange={(e) => {
                      if (e.target.checked) {
                ...
                <TableRow key={chainer.id}>
                  <TableCell>
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedChainers([...selectedChainers, chainer.id]);
                        } else {
                          setSelectedChainers(selectedChainers.filter(id => id !== chainer.id));
                        }
                      }}
                    />
                  </TableCell>
                  <TableCell>
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{chainer.userEmail}</div>
                  ...
                  <TableCell>
                    <div>
                      <div className="font-medium">{chainer.totalReferrals} referrals</div>
                      <div className="text-sm text-gray-500">
                        ${chainer.totalRaised.toLocaleString()} raised
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div>
                      <div className="font-medium">${chainer.commissionEarned.toLocaleString()}</div>
                      <div className="text-sm text-gray-500">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <span className={`font-medium ${getFraudScoreColor(chainer.fraudScore)}`}>
                        {chainer.fraudScore}%
                      </span>
                      {chainer.suspiciousActivity && (
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleChainerAction(chainer.id, 'view')}
                      >
                      ...
                      {chainer.status === 'active' ? (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleChainerAction(chainer.id, 'suspend')}
                        >
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleChainerAction(chainer.id, 'activate')}
                        >
