              ...
              {isLoadingMore && (
                ...
                >
                  {Array.from({ length: viewMode === "grid" ? 4 : 2 }).map((_, index) => (
                    <div
                      key={`campaigns-loadmore-skeleton-${index}`}
              ...
              {hasMore && (
                <div className="text-center mt-8">
                  <Button
                    ...
                    className="rounded-full bg-[#104109] px-8 py-4 h-auto text-base font-bold"
                  >
                    {isLoadingMore ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-[#ffffff]"></div> : "Load More"}
                  </Button>
                </div>
              )}
            </>
